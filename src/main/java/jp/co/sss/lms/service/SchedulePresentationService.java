...
public class SchedulePresentationService {
	...
	DateUtil dateUtil;
	@Autowired
	MLmsUserRepository mLmsUserRipository;
	@Autowired
	...
	private PresentationPlaceDto createPresentationPlaceDto(TPresentationPlace tPresentationPlace) {
		...
		if (tPresentationPlace.getMPresentationSchedule() != null) {
			...
			// ◆受講中の受講生 ★未編集
			List<MLmsUser> mLmsUserList = mLmsUserRipository.findStudentByPlaceId(tPresentationPlace.getMPlace().getPlaceId(), mPresentationSchedule.presentationDate);
			...
			for (MLmsUser mLmsUser : mLmsUserList) {
				...
				BeanUtils.copyProperties(mLmsUser, mLmsUser.mUser);
